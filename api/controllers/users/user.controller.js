...
module.exports = {
  createUser: (req, res) => {
    const body = req.body;
    if (body.name && body.firebase_token && body.email) {
      const validateEmail = req.body.email.split("@");
      if (validateEmail[1] == domain) {
        pool.query(
          `select * from users where email = '${req.body.email}' and name = '${req.body.name}'`,
          ...
          (error, results, fields) => {
            if (results && results[0]) {
              ...
              if (
                configSuperAdminEmail1 == results[0].email ||
                configSuperAdminEmail2 == results[0].email
              ) {
                var roleId = 1;
              }
        ...
        const message = `Please provide a valid email with ${domain} domain.`;
  ...
  userLogin: (req, res) => {
    ...
    if (email) {
      ...
      if (validateEmail[1] == domain) {
        userService.userLogin(email, (err, results) => {
          ...
          if (results.length > 0) {
            ...
            db.query(
              ...
              [results[0].id, token, created, created]
            );
        ...
        const message = "Please provide an email.";
        return errorResponse(res, 400, false, message);
    }
  },
